<?php

declare(strict_types=1);

namespace Tests\Feature;

use Tests\TestCase;

class LogoProvidedDataUriRegressionTest extends TestCase
{
    private const DATA_URI = 'data:image/png;base64,';

    private const RAW_BASE64 = '';

    public function test_large_data_uri_logo_renders(): void
    {
        $response = $this->get('/?username=regression-data-uri&logo=' . urlencode(self::DATA_URI));
        $response->assertStatus(200);
        $response->assertHeader('Content-Type', 'image/svg+xml');
        $this->assertStringContainsString('<image', $response->getContent());
    }

    public function test_large_raw_base64_logo_renders(): void
    {
        $response = $this->get('/?username=regression-raw-base64&logo=' . urlencode(self::RAW_BASE64));
        $response->assertStatus(200);
        $response->assertHeader('Content-Type', 'image/svg+xml');
        $this->assertStringContainsString('<image', $response->getContent());
    }
}
